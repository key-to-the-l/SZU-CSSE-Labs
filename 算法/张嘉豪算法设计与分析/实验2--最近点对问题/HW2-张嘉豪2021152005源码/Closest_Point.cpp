...
double Brute_force(Point *p,int num)
{
    double result=10000;
    int a,b;
    ...
    cout<<"Brute force time cost: ";
    cout<<cost_time<<"ms"<<endl;

    cout<<"ClosestPair: "<<"("<<p[a].x<<","<<p[a].y<<")"<<"--";
...
double closest(Point *p,int low,int high) 
{
    if (low==high)      //1个点
    return 10000000;
    if(high-low==1)     //2个点
    ...
    int index=0;                        //集合计数
    //Point *temp=new Point[high];           //点集合

    left_min=closest(p,low,mid);        //递归求左区域最小距离
    right_min=closest(p,mid+1,high);    //右区域
    if(left_min==0)
    left_min=100000;
    else if(right_min==0)
    right_min=100000;
...
int main()
{
    ...
    //cout<<"num="<<num<<" point_arrary:"<<endl;    //测试点对
    
    random(p,num);
    cout<<Brute_force(p,num)<<endl;
    cout<<endl;
    ...
    sort(p,p+num,cmp_x);
    double ans=closest(p,0,num-1);
    
    time_end=clock();
    double cost_time=(double)(time_end-time_start)*1000/CLOCKS_PER_SEC;
    cout<<"Divide algorithm time cost: ";
    cout<<cost_time<<"ms"<<endl;
    cout<<"ClosestDistance: "<<ans<<endl;
