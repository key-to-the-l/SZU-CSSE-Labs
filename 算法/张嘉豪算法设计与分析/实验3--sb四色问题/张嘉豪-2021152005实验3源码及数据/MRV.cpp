...
bool checkNode(int num,int clr)                        //判断num点能否涂clr这种颜色
{
    for(int i=1;i<=n;i++)  
    {
        ...
        if(checkNode(Coloring_point,i))      
        {
            ...
            for(int j=1;j<=n;j++)           //对邻接点的度数,可涂颜色数进行修改
            {
                if(matrix[Coloring_point][j]==1)
                {
                    ...
                    node[index].enableColor--;  //颜色--
                }
            }

            MRV(num+1);     //开始涂下一个点,即进入下一层
...
int main()
{
   ...
   //input1();
    ...
    cout<<fixed<<setprecision(6)<<"The verage time of the MRV="<<cost_time<<"ms"<<endl;
